...
def clean_mermaid_code(mermaid_code: str) -> str:
    """
    Clean and lightly repair Mermaid code generated by vision models.

    Handles:
    - ```mermaid fences
    - accidental first line: mermaid
    - one-line Mermaid separated by semicolons
    - graph/flowchart edges with unsafe labels like:
      Input Embedding --> Add & Norm
    """
    ...
    # Convert one-line Mermaid into normal multiline Mermaid.
    ...
    # Remove accidental language label from model output.
    if lines and lines[0].lower() == "mermaid":
        lines = lines[1:]
    ...
    if not lines[0].startswith(valid_starts):
        return ""

    # Only auto-repair graph / flowchart syntax.
    if not (lines[0].startswith("graph ") or lines[0].startswith("flowchart ")):
        return "\n".join(lines)

    def escape_label(label: str) -> str:
        return label.replace('"', "'").strip()

    def is_safe_id(text: str) -> bool:
        return bool(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", text.strip()))

    def is_already_node(text: str) -> bool:
        text = text.strip()
        return bool(
            re.match(
    ...
    def split_edge(statement: str) -> tuple[str, str, str] | None:
        for arrow in ["-->", "-.->", "==>", "---"]:
            if arrow in statement:
                left, right = statement.split(arrow, 1)
                left = left.strip()
                right = right.strip()
                edge = arrow
                ...
                if right.startswith("|") and "|" in right[1:]:
                    edge_label, actual_right = right[1:].split("|", 1)
                    edge = f"{arrow}|{edge_label.strip()}|"
                    right = actual_right.strip()
    ...
    # Count labels so repeated unsafe labels like "Add & Norm"
    # can become separate nodes instead of being merged incorrectly.
    label_counts: dict[str, int] = {}
    for line in lines[1:]:
        edge = split_edge(line)
        if not edge:
            continue
        left, _, right = edge
        for label in [left, right]:
            if not is_already_node(label):
                label_counts[label] = label_counts.get(label, 0) + 1

    node_counter = 0
    subgraph_counter = 0
    current_scope_nodes: dict[str, str] = {}

    def new_node_id(label: str) -> str:
        nonlocal node_counter
        node_counter += 1

        base = re.sub(r"[^A-Za-z0-9_]+", "_", label).strip("_").lower()
        if not base:
            base = "node"
        ...
        return f"{base}_{node_counter}"

    def node_ref(label: str, *, side: str) -> str:
        label = label.strip()

        if is_already_node(label):
            return label

        if is_safe_id(label):
            return label

        repeated_unsafe_label = label_counts.get(label, 0) > 1

        # For repeated labels:
        # - left side uses latest node
        # - right side creates a new node
        if side == "left" and label in current_scope_nodes:
            return current_scope_nodes[label]

        if side == "right" and label in current_scope_nodes and not repeated_unsafe_label:
            return current_scope_nodes[label]

        node_id = new_node_id(label)
        current_scope_nodes[label] = node_id

        return f'{node_id}["{escape_label(label)}"]'
    ...
    for line in lines[1:]:
        ...
        if lower.startswith("subgraph "):
            ...
            elif is_safe_id(title):
                cleaned_lines.append(f'subgraph {title}["{escape_label(title)}"]')
            else:
                cleaned_lines.append(
                    f'subgraph subgraph_{subgraph_counter}["{escape_label(title)}"]'
                )
        ...
        left, arrow, right = edge
        left_node = node_ref(left, side="left")
        right_node = node_ref(right, side="right")

        cleaned_lines.append(f"    {left_node} {arrow} {right_node}")
...
def render_visual_source(chunk: dict[str, Any], index: int) -> None:
    ...
    vision_description = (
        chunk_value(chunk, "vision_description", "")
        or chunk_value(chunk, "description", "")
    )

    invalid_vision_descriptions = {
        "",
